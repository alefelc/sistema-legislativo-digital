class ResolucionsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.json { render json: ResolucionDatatable.new(view_context) }
    end
  end
  ...
  def new
    @resolucion = Resolucion.new
    respond_to do |format|
      format.html {render partial: "modal_resolucion", locals: { actionvar: "create"}}
    end
  end
  ...
  def edit
    @resolucion = Resolucion.find(params[:id])
    respond_to do |format|
      format.html {render partial: "modal_resolucion", locals: { actionvar: "update"}}
    end
  end
  ...
  def create
    ...
    @resolucion = Resolucion.create resol.to_hash
    ...
    unless params[:clasificaciones].blank?
      params[:clasificaciones].each { |key,value| @resolucion.clasificacions <<  Clasificacion.where(nombre: key).first() }
    end
    ...
    unless params[:linked_normas].blank?
      JSON.parse(params['linked_normas']).each do |key, value|
        @resolucion.relationship_modificadas.create do |rm|
    ...
    unless params[:destinies].blank?
      JSON.parse(params[:destinies]).each do |key, value|
        @resolucion.destinos.create do |d|
          d.tipo = value['tipo']
          d.fecha = value['fecha']
  ...
  def update
    ...
    @resolucion.update resol.to_hash

    ## update params tags_ids the PATCH
    current_tags = params[:tags_ids].split(',')
    old_tag = @resolucion.tags.map{ |x| x.id.to_s }
    ...
    current_clasific = []
    old_clasific = @resolucion.clasificacions.map{ |x| x.id}
    ...
    if params['linked_normas'].present?
      ...
      current_normas = []
      old_normas = @resolucion.modificadas.map{ |x| x.id }
      JSON.parse(params['linked_normas']).each do |key, value|
      ...
      # delete norms
      (old_normas - current_normas).each { |id| @resolucion.modificadas.delete(id) }
    end
    ...
    old_destinies = @resolucion.destinos.map{ |x| x.id.to_s }
    new_destinies.each do |key, value|
      @resolucion.destinos.create do |nd|
        nd.tipo = value['tipo']
        nd.fecha = value['fecha']
        nd.destino = value['destino']
      end
    end
    (old_destinies - current_destinies).each { |id| @resolucion.destinos.delete(id) }
  ...
  def resolucion_params
    params.require(:resolucion).permit("letra", "nro", "bis", "descripcion",
          "sumario", "sancion", "observaciones", "entrada_vigencia", "finaliza_vigencia",
